...
### Requirement: 填写者详情视图
后台 SHALL 在正常 `Respondents` 视图中仅列出未删除填写者及其单位、首次创建时间和最后修改时间，并 SHALL 允许管理员查看一份完整的个人答卷。答卷详情 SHALL 按当前问卷目录展示答案，并 SHALL NOT 展示已移除题目的历史答案。每条记录 SHALL 提供 `Delete` 操作。
...
后台 SHALL 将当前未删除答卷数据导出为 UTF-8 CSV，其中包含身份信息、首次创建与最后修改时间，以及当前问卷目录中每道题目对应的清晰列名。导出列 SHALL 只按当前精简后的目录生成；已移除题目的历史答案 SHALL 保留在数据库中但 SHALL NOT 出现在导出中。系统 SHALL 对可能被电子表格软件解释为公式的字段值进行安全处理。
...
#### Scenario: 题目精简后的导出列
- **WHEN** 管理员在题目精简后导出答卷，且某些答卷仍保存着已移除题目的历史答案
- **THEN** 导出 SHALL 只包含当前目录的题目列，并且 SHALL NOT 包含已移除题目或其历史答案
